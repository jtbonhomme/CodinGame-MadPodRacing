...
func main() {
    ...
    for {
        ...
        fmt.Fprintf(os.Stderr, "x: %d; y: %d, d: %d, a: %d\n", int(x), int(y), int(nextCheckpointDist), int(nextCheckpointAngle))
        ...
        fmt.Fprintf(os.Stderr, "cpx: %d; cpy: %d, compute a: %d\n", int(nextCheckpointX), int(nextCheckpointY), int(angle))
        p = Point{x, y}
